...
ControlTime::ControlTime(std::string filename) {
    ...
    if(!count)
        csv << "compacted_size,original_size,func,mode,user_time,system_time,clock_time" << std::endl;
}
...
void ControlTime::write_csv(long compacted_size, long original_size, std::string func, std::string mode, Time time) {
    // Escreve no arquivo CSV
    csv << compacted_size << "," << original_size << "," << func << "," << mode << "," << time.user_time << "," << time.system_time << "," << time.clock_time << std::endl;
}