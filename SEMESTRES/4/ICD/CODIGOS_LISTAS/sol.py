...
df_novo = df.fillna(df.mean(numeric_only=True))
df_novo
...
above_60_low_hours.mean(numeric_only=True)
...
def high_delay(df):
    ...
    not_cancelled = df.query("Cancelled == 0")
    not_cancelled["Time"] = np.zeros(not_cancelled.shape[0])
    
    same_day = not_cancelled["CRSArrTime"] >= not_cancelled["CRSDepTime"]
    not_cancelled.loc[same_day, "Time"] = not_cancelled["CRSArrTime"] - not_cancelled["CRSDepTime"]
    not_cancelled.loc[~same_day, "Time"] = 2400 - not_cancelled["CRSArrTime"] + not_cancelled["CRSDepTime"]

    info = not_cancelled[['OriginAirportName', 'DestAirportName', 'Time']]
    result = info.groupby(["OriginAirportName", "OriginAirportName"])["Time"].mean()
...
nice = df.query("Cancelled == 0")
plt.scatter(nice["DepDelay"], nice["ArrDelay"], edgecolor='k', alpha=0.75)
plt.xlabel('DepDelay')
plt.ylabel('ArrDelay')
